...
**Status (v0.1 beta):** every lifecycle path is now live-verified in Orca against a real sandbox — first-workspace create → connect → terminals → Claude Code running; second-workspace reuse (same VM, no re-clone, one more linked worktree); sleep/wake, including waking after the sandbox had been stopped; refcounted destroy (the VM survives while a sibling workspace remains, and is removed with the last one); and the keepalive's identity check, verified by pointing a pidfile at an unrelated process and confirming the recipe respawns without signalling it. Expect rough edges and report them.
...
- **Plain TCP on a stable loopback port.** The `sshd` port is published to a deterministic host port in `30000`–`39999`, derived from a project hash (e.g. `sbx ports <name> --publish 33792:2222`), so it survives VM restarts. If that port is taken — a colliding project or an unrelated process — the recipe walks forward through the range (up to 10 ports) and reuses whatever mapping is already published on later runs; the actual port always flows back to Orca in the connection JSON. Orca connects over plain TCP with the project key — no SSH proxy in the path, so Orca's connection multiplexing (ControlMaster) just works.
- **A keepalive against idle auto-stop.** Direct-TCP traffic is invisible to the `sbx` daemon's idle tracking, so the recipe holds one lightweight `sbx exec` session open per project to keep the sandbox from being auto-stopped while a workspace is connected. Its pid is recorded under `~/.orca-sbx/<name>/`, and nothing trusts that number on its own: a pidfile is believed only while the recorded process is still running *that* session, since after a crash or a reboot the OS may have recycled the number onto something unrelated — which would otherwise cost you a respawn the sandbox needed, or send a stranger's process the SIGTERM meant for the keepalive.
